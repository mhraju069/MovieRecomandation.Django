...
from .models import FeedPost, UserPrefrences, ReviewAndRating, WatchedMovies
        ...
        response = [
            {
                ...
                "rating": get_tmdb_overall_rating(i.get("id")),
...
def get_personalized_tonight_trending(user, media_type="movie"):
    media_type = normalize_media_type(media_type)
    titles, rating_signals = _filter_unseen_trending(user, media_type)
    onboarding_genre_ids = _get_onboarding_genre_ids(user)
    rating_count = rating_signals["count"]

    if rating_count == 0:
        ranked_titles = _apply_mixed_new_user_ranking(titles, onboarding_genre_ids)
    elif rating_count < 5:
        ranked_titles = _apply_light_history_ranking(titles, onboarding_genre_ids)
    else:
        scored_titles = []
        for title in titles:
            title_genre_ids = set(title.get("genre_ids", []))
            score = 0

            if title_genre_ids.intersection(rating_signals["highly_rated_genres"]):
                score += 3
            if title_genre_ids.intersection(onboarding_genre_ids):
                score += 2
            if title_genre_ids.intersection(rating_signals["low_rated_genres"]):
                score -= 2
            if title.get("tmdb_rank", title.get("rank", 999)) <= 10:
                score += 1

            scored_titles.append((score, title.get("tmdb_rank", title.get("rank", 999)), title))

        scored_titles.sort(key=lambda item: (-item[0], item[1]))
        ranked_titles = [item[2] for item in scored_titles[:10]]
...
def _get_user_preference_ids(user):
    ...
    for prefrence in prefrences:
        if isinstance(prefrence.genre, list):
            for genre in prefrence.genre:
                genre_id = genre.get("id") or genre.get("genre_id") if isinstance(genre, dict) else genre
                try:
                    genre_ids.add(int(genre_id))
                except (TypeError, ValueError):
                    continue
...
def get_because_you_liked_recommendations(user, media_type="movie", extra_blocked_ids=None, limit=10):
    ...
    ranked_titles = _score_because_you_liked_candidates(
        candidates,
        user,
        media_type,
